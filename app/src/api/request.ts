...
export const api = {
  ...
  getReportsByEpisode: (episodeId: string) => request(`/reports?episodeId=${encodeURIComponent(episodeId)}`),
  ...
  exportFollowup: (episodeId: string, format: string) => request('/followup/export', { method: 'POST', data: { episodeId, format } }),
  ...
  getSessionHistory: (episodeId?: string) => request(`/qa/history${episodeId ? `?episodeId=${encodeURIComponent(episodeId)}` : ''}`),
  safetyCheck: (text: string) => request('/safety/check', { method: 'POST', data: { text } }),
  createFeedback: (data: any) => request('/feedback', { method: 'POST', data }),
  getFeedbackList: (isErrorReport?: boolean) => request(`/feedback?isErrorReport=${isErrorReport === undefined ? '' : isErrorReport}`),
};
