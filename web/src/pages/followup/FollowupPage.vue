        ...
        <template v-else>
          <section class="summary-card">
            ...
            <div class="section-block">
              ...
              <p class="block-text">{{ sections.chiefComplaint?.onsetDate || '尚未确认' }} 开始腰痛，具体日期不确定；起初以久坐后酸痛为主。</p>
              <div class="tag-row">
            ...
            <div class="section-block">
              ...
              <p class="block-text">每日步行约 20 分钟、热敷；避免久坐；未使用药物。</p>
              <div class="tag-row">
                <span class="tag tag-source">自述</span>
              </div>
            </div>
...
const chiefText = computed(() => {
  ...
  const last = logs[0];
  if (!last) return '尚未确认';
  return `目前腰痛持续约 1 个月，最近 1 周加重；主要在左侧；能坐约 ${last.sitMinutes ?? '尚未确认'} 分钟；夜间痛醒 1 次/晚。`;
});
